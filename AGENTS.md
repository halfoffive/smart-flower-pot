...
- **BLE connection interval parameters**: `setMinPreferred(0x06)` sets minimum connection interval to ~7.5ms; `setMaxPreferred(0x12)` sets maximum to ~22.5ms. Both must be called correctly — a past bug duplicated `setMinPreferred` instead of calling `setMaxPreferred`.
...
- **BLE disconnect listener must be removed before re-adding**: `web/src/lib/ble.js`'s `handleDisconnect` now calls `savedDevice?.removeEventListener('gattserverdisconnected', handleDisconnect)` before `cleanup()`, preventing duplicate listeners from accumulating across reconnection attempts. Same in `attemptReconnect()` — the old listener is removed before adding the new one.
- **Serial `waitForResponse` now guards against concurrent calls**: `web/src/lib/serial.js`'s `waitForResponse()` rejects immediately if `pendingResponse` is already set, preventing the "two calls overwrite each other" race. The `pendingResponse` is nulled both in the timeout handler and in the success resolver.
- **Theme system listener uses named function**: `web/src/composables/useTheme.js` stores the system theme change handler in a named function `onSystemThemeChange` instead of an anonymous arrow function, so `removeEventListener` actually removes it. Previously every `setTheme('light'/'dark')` call leaked a listener.
- **Global error handler registered**: `web/src/main.js` sets `app.config.errorHandler` for unhandled errors (async rejections, timer exceptions). `App.vue`'s `onErrorCaptured` returns `true` (not `false`) which correctly prevents error propagation to the global handler.
...
- **Tauri 图标从源 PNG 生成**：`desktop/public/potted_plant_3d.png` 是图标源文件（1024×1024 PNG），每次构建前通过 `bun tauri icon public/potted_plant_3d.png` 重新生成所有平台图标。如需更换应用图标，替换此 PNG 文件后重新生成即可。
...
- **图标自动生成**：构建前执行 `bun tauri icon public/potted_plant_3d.png`，从源 PNG 生成全平台图标（含 Android mipmap），确保各平台图标一致性
