...
- **抗噪音帧解析**：客户端帧解析器内置 type+payloadLen 合法性校验和 XOR 校验和双重机制，即便固件的串口调试文本与二进制帧混用，也能正确识别并处理有效帧
...
>
> **关于串口噪音**：固件在发送二进制帧的同时，可能通过同一串口输出人类可读的调试文本（如传感器读数、系统状态等）。这些文本可能包含与帧头 `0xAA 0x55` 巧合匹配的字节序列。客户端帧解析器通过 **type+payloadLen 合法性校验** 和 **XOR 校验和** 双重机制识别并跳过噪音中的虚假帧头，不会因此而卡死或解析错误。
